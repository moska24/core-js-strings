...
function sumOfCodes(str) {
  ...
  codeSum = code.reduce((sum, el) => sum + el, 0);
  return codeSum;
}
...
function countVowels(str) {
  const VowelsMas = str.match(/[aeiouyAEIOUY]/gi);
  if (VowelsMas === null) {
    return 0;
  }
  const count = VowelsMas.length;
  return count;
}
...
function isPalindrome(str) {
  const strWithoutSpaces = str.replace(/[^a-zа-яё]/gi, '').toLowerCase();
  const backStr = strWithoutSpaces.split('').reverse().join('');
  if (strWithoutSpaces === backStr) {
    return true;
  }
  return false;
}
...
function extractNameFromTemplate(value) {
  ...
  return `${newString.split(' ').slice(1).join(' ')}`;
}
...
function extractEmails(str) {
  const separator = ';';
  const arrayOfEmail = str.split(separator);
  return arrayOfEmail;
}
...
function encodeToRot13(str) {
  ...
  const codeSymbols = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let outputStr = '';
  for (let i = 0; i < str.length; i += 1) {
